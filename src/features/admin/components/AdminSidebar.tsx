    ...
    <>
      {isCollapsed && (
        <button
          ...
          className="fixed left-4 z-10 flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200/80 bg-white/85 text-slate-600 shadow-lg shadow-slate-300/30 backdrop-blur-xl transition-all hover:bg-white hover:text-slate-900"
          style={{ top: "calc(env(safe-area-inset-top, 0px) + 15px)" }}
          aria-label="Mo rong thanh ben"
          title="Mo rong thanh ben"
        >
      ...
      {/* Sidebar */}
      {!isCollapsed && (
        <aside className="fixed lg:sticky top-0 left-0 z-40 h-screen w-64 flex flex-col border-r border-white/60 bg-white/80 backdrop-blur-xl shadow-2xl shadow-slate-200/50 transition-all duration-300">
          <div className="flex h-14 shrink-0 items-center justify-center border-b border-white/40 px-3">
            <button
              ...
              aria-label="Thu gon thanh ben"
              title="Thu gon thanh ben"
            >
        ...
        <div className="shrink-0 border-t border-white/40 p-3 flex flex-col gap-2">
          ...
          <form action="/api/admin/logout" method="POST">
            <button
              type="submit"
              className="flex w-full items-center gap-3 rounded-2xl px-3 py-3 text-sm font-semibold text-slate-600 transition-all hover:bg-red-50/80 hover:backdrop-blur-sm hover:text-red-700"
            >
